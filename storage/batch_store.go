...
var (
	...
	// RUID to BatchID: ri:{ruid} -> batchId
	prefixRUIDIndex = []byte("ri:")
)
...
func (s *Store) SaveBatchMeta(meta *types.BatchMeta) error {
	...
	// Save RUID index entries
	for _, ruid := range meta.EventRUIDs {
		ruidKey := append(prefixRUIDIndex, ruid[:]...)
		if err := batch.Put(ruidKey, []byte(meta.BatchID)); err != nil {
			return err
		}
	}
...
// GetBatchByRUID finds a batch by RUID
func (s *Store) GetBatchByRUID(ruid [32]byte) (*types.BatchMeta, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()

	key := append(prefixRUIDIndex, ruid[:]...)
	batchIDBytes, err := s.db.Get(key)
	if err != nil {
		return nil, ErrNotFound
	}
