...
func (api *API) VerifyRUID(ctx context.Context, ruidHex string) (*VerifyResult, error) {
	...
	defer otsmetrics.VerificationTimer.UpdateSince(verifyStart)
	...
	meta, err := api.store.GetBatchByRUID(ruidBytes)
	if err != nil {
		otsmetrics.IncVerification(false)
		return &VerifyResult{
			RUID:     ruidHex,
			Verified: false,
			Message:  "RUID not found in any batch",
		}, nil
	}

	// 2. Get attempt status
	attempt, err := api.store.GetAttempt(meta.BatchID)
	if err != nil {
		otsmetrics.IncVerification(false)
		return &VerifyResult{
			RUID:     ruidHex,
			Verified: false,
			BatchID:  meta.BatchID,
			Message:  "batch attempt not found",
		}, nil
	}

	// 3. Check if batch is confirmed/anchored
	if attempt.Status != ots.AttemptStatusConfirmed && attempt.Status != ots.AttemptStatusAnchored {
		otsmetrics.IncVerification(false)
		return &VerifyResult{
			RUID:     ruidHex,
			Verified: false,
			BatchID:  meta.BatchID,
			Message:  "batch not yet confirmed on Bitcoin (status: " + attempt.Status.String() + ")",
		}, nil
	}

	// 4. Rebuild Merkle tree and verify proof
	tree, err := merkle.BuildFromRUIDs(meta.EventRUIDs)
	if err != nil {
		otsmetrics.IncVerification(false)
		return &VerifyResult{
			...
			Verified: false,
			BatchID:  meta.BatchID,
			Message:  "failed to rebuild Merkle tree: " + err.Error(),
		}, nil
	}
	...
	if err != nil {
		...
		return &VerifyResult{
			...
			Verified: false,
			BatchID:  meta.BatchID,
			Message:  "failed to generate proof: " + err.Error(),
		}, nil
	}
	...
	// 7. Verify root matches stored root
	...
	// Success!
	otsmetrics.IncVerification(true)
	return &VerifyResult{
		RUID:           ruidHex,
		Verified:       true,
		BatchID:        meta.BatchID,
		...
		Message:        "RUID verified and anchored on Bitcoin",
	}, nil
}
